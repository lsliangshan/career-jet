  ...
  <view class="w-[100vw] h-[100vh] flex flex-row items-end justify-center">
    ...
    <Layout :hasHeader="true">
      <view class="relative w-full h-full flex flex-col">
        ...
        >
          <view
            class="sticky top-0 left-0 z-[9] w-full h-[88rpx] bg-[#fff] border-b border-b-[1rpx] border-[#F0F0F0] flex flex-row items-center justify-center"
          >
          ...
          <view class="relative w-full pb-[24rpx] box-border">
            ...
            <view
              class="absolute top-0 left-0 z-[9] w-full bg-[#fff] transition-opacity duration-300 flex flex-col items-center justify-center gap-[24rpx]"
              :class="[regeneratingVisible ? 'opacity-100' : 'opacity-0']"
              :style="{
                height: `calc(100% - 128rpx - 100rpx - ${safeBottom}px)`,
              }"
              v-if="isRegenerating"
            >
              <div class="spinner">
          ...
          <view
            class="sticky bottom-0 left-0 z-[9] w-full bg-[#fff] border-t border-t-[1rpx] border-[#f0f0f0] flex flex-row items-center justify-center gap-[24rpx]"
            :style="{
          ...
          >
            ...
            >
              <text class="text-[32rpx] text-[#fff]">确认</text>
            </view>
          </view>
        </scroll-view>
      </view>
...
import { IConfirmRoleInfo, IConfirmStoryInfo } from "./types";
...
async function doConfirmStory() {
  ...
  // 确认故事
  const { data } = await requestCustomUrl({
    url: "https://wf.qyflows.com/webhook-waiting/617847/pb-confirm-story",
    method: "POST",
    data: {
      id: props.info.id,
      title: renderStoryTitle.value,
  ...
  emit("on-confirm", {
    action: data.action as EConfirmAction,
    data: data.data as IConfirmRoleInfo,
  });
}
</script>
